...
- **팀 협업 프로세스 최적화 및 리더십**
  - 정기 회의와 작업 분배를 통한 프로젝트 일정 관리 및 팀원 간 명확한 역할 분담
...
### 피드 콘텐츠 필터링
  ...
  - `추천` 탭: 내 학습 언어를 모국어로 하고 내 모국어를 학습하는 사용자들의 게시물 필터링
  - `동급생` 탭: 나와 동일한 모국어와 학습 언어 조합을 가진 사용자들의 게시물 표시
  - `근처` 탭: 동일한 읍면동에 거주하는 사용자들의 게시물만 표시
...
- **Firestore 보안 규칙 기반 데이터 접근 제어**
  - Firestore Security Rules를 통한 사용자별 데이터 소유권 검증 시스템 구현
  - 좋아요와 댓글 시스템에 대한 세밀한 권한 관리로 본인 데이터만 수정/삭제 가능하도록 제한
  - 읽기 권한은 모든 사용자에게 허용하되 쓰기 권한은 엄격한 인증 기반으로 제한하여 데이터 보안성 확보
