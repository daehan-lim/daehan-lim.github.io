...
- 쿠바의 열악한 네트워크 환경(높은 데이터 비용, 불안정한 연결)과 웹사이트 기반 뉴스 플랫폼의 한계를 극복하기 위해 모바일 최적화된 뉴스 앱 개발의 필요성을 확인
- 기존 웹사이트는 사용자가 관심 있는 주제의 뉴스를 찾는 데 평균 2분 이상이 소요되었고, 맞춤형 뉴스 소비가 사실상 불가능한 상황
- 데이터 사용량 절약과 오프라인 접근성을 통해 쿠바 사용자들에게 실질적인 가치를 제공하고, 개인화된 뉴스 경험을 통한 정보 접근성 향상을 목표로 주제 선정
